<?php 
	if(isset($_POST['Dress'])){
		...
		if(empty($company) OR empty($model) OR empty($details) OR empty($price)){
			...
			$Dress_query = "INSERT INTO tb_Dress (company,model,details,price) VALUES ('$company','$model','$details','$price')";
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title" id="myModalLabel"><?php echo $Dress['company'].' &nbsp; '.$Dress['model']; ?></h4>
      </div>
      <div class="modal-body">
        <?php 
        	echo '<h2>Brand Name # '.$Dress['company'].'</h2>';
        	echo '<h2>Model # '.$Dress['model'].'</h2>';
        	echo '<h4>Price # '.$Dress['price'].'.tk</h4>';
