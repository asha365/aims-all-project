<?php 
	if(isset($_POST['Book'])){
		...
		if(empty($company) OR empty($model) OR empty($details) OR empty($price)){
			...
			$Book_query = "INSERT INTO tb_Book (company,model,details,price) VALUES ('$company','$model','$details','$price')";
...
<?php 
	$Book_load_qry = "SELECT * FROM tb_Book";
	$result = $DB->select($Book_load_qry); 
	if($result){
		$i = 0; 
...
?>
	<tr>
		<td><?php echo $i; ?></td>
		<td><?php echo $Book['company']; ?></td>
		<td><?php echo $Book['model']; ?></td>
		<td><?php echo $Book['price']; ?></td>
		<td>
			<a href="" class="btn btn-info btn-sm" data-toggle="modal" data-target="#mobile_view<?php echo $Book['id']; ?>">View</a>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title" id="myModalLabel"><?php echo $Book['company'].' &nbsp; '.$Book['model']; ?></h4>
      </div>
      <div class="modal-body">
        <?php 
        	echo '<h2>Brand Name # '.$Book['company'].'</h2>';
        	echo '<h2>Model # '.$Book['model'].'</h2>';
        	echo '<h4>Price # '.$Book['price'].'.tk</h4>';
        	echo 'Specification # '.$Book['details']; 
        ?>
      </div>
