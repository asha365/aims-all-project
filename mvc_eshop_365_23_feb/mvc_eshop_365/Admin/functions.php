...
/*..------------------------------------------------ PAGE SAVE MODULE-------------------------------------*/
    if(isset($_POST['post_save'])){
        ...
        if(empty($post_page_id) OR empty($post_content)){
                echo '<script>window.location="posts.php?error"</script>'; ;  
        }else{
            $post_save_qry = "INSERT INTO tb_mvc_365_posts(post_page_id,post_title, post_content,post_tags)VALUES('$post_page_id','$post_title','$post_content','$post_tags')";

            $post_save_result = $DB->insert($post_save_qry);
            if ($post_save_result) {
                echo '<script>window.location="posts.php?success"</script>'; 
            }
        }
    }
/*
...
|AIMS-365 -----------------------------------------------------------------------------------------AIMS-365|
|AIMS-365 -----------------------------------------------------------------------------------------AIMS-365|
===========================================pagelist option module==========================================
*/ 

/*---------------------------------------postlist site STATUS ON---------------------------------------------*/
    if(isset($_GET['postlist_status_on'])){
        $postlist_status_on = $_GET['postlist_status_on']; 
        $postlist_on_qry = "UPDATE tb_mvc_365_posts SET post_status=0 WHERE post_id='$postlist_status_on'"; 
        $postlist_result_on = $DB->update($postlist_on_qry); 
        if($postlist_result_on){
            echo '<script>window.location="postlist.php?success"</script>';
        }
    }
/*------------------------------------------postlist site STATUS OFF------------------------------------------*/
    if(isset($_GET['postlist_status_off'])){
        $postlist_status_off = $_GET['postlist_status_off']; 
        $qry_postlist_off = "UPDATE tb_mvc_365_posts SET post_status=1 WHERE post_id='$postlist_status_off'"; 
        $result_postlist_off = $DB->update($qry_postlist_off); 
        if($result_postlist_off){
            echo '<script>window.location="postlist.php?success"</script>';
        }
    }
/*-----------------------------------------postlist delete option module-----------------------------------------*/

 if(isset($_GET['postlist_delid'])){
        $postlist_delid = $_GET['postlist_delid'];
        $qry_postlist_delid = "DELETE FROM  tb_mvc_365_posts WHERE post_id = '$postlist_delid'";
        $result_postlist_delid  = $DB->delete($qry_postlist_delid);
        if($result_postlist_delid){
            echo '<script>window.location="postlist.php?success"</script>';
        }
    }
/*
...
if(isset($_GET['pedit_id'])){
    $peid = $_GET['pedit_id'];   
    $get_post_edit_qry = "SELECT * FROM tb_mvc_365_posts WHERE post_id='$peid'"; 
    $get_post_edit_result = $DB->select($get_post_edit_qry); 
    if($get_post_edit_result){
        $get_post_edit_data = $get_post_edit_result->fetch_assoc(); 
    }
}
/*
|AIMS-365 -----------------------------------------------------------------------------------------AIMS-365|
...
if(isset($_POST['post_update'])){
    ...
    if(empty($post_id) OR empty($post_page_id)){
        echo '<script>window.location="postedit.php?error&pedit_id='.$post_id.'"</script>';
    }else{
        $post_update_qry = "UPDATE tb_mvc_365_posts 
        SET 
        ...
        $post_uupdate_result = $DB->update($post_update_qry); 
        if($post_uupdate_result){
            echo '<script>window.location="postedit.php?success&pedit_id='.$post_id.'"</script>';
        }
    }
}
